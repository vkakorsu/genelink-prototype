...
Video walkthrough: [youtu.be/uH3tDyWwJwk](https://www.youtube.com/watch?v=uH3tDyWwJwk)
...
| **A lapsed clock never grants (R8), and a lawful one is not mistaken for a lapse.** The second Kenya case sits in `deadline_lapsed` with a remedy against the administrator and no permit. The linter refuses any clock whose lapse target is a granted state, and any clock that could run out in a state with no declared lapse. Recording an external instrument is refused unless the machine is in a granted state or an awaiting-record slot already exists. Clocks are counted as the law counts them, and every clock carries its counting basis with an evidence class: days are dates on the authority's own calendar in the time zone the country file names, the day of the event is not counted, and a deadline runs to the end of its last day there; working days skip each country's gazetted public holidays (a calendar in its file); a clock starts where the law starts it (Kenya's 30 working days from receipt, reg. 14(1)); a suspension stops the count and the answer resumes it (Kenya reg. 14(3)); a clock runs through every state the law runs it through (Colombia's 30 working days from registration, D391 Art. 29); where the start is not visible the clock counts to the latest day the law allows and says so (Brazil's 60 days from SisGen's notification, counted as 65 from the form); and the authority's extension of up to 60 working days on top of the 30 is recorded as its act, capped by the file. Nobody records a lapse by hand, the administrator included: it follows from the clock check once the last day has ended, and a hand-posted lapse is refused and audited. The demo's time control ("Check at +45 days", "Force lapse now") marks every check it makes as simulated in the audit chain. | Case `case_3_ke`; the administrator's extension control, which appears on a Colombian case while its Art. 29 clock runs, from admission onwards (no seeded case sits there: take a new Colombian match to admission, and the control is on its Regulator processing panel); `core/engine/stateMachine.ts`, `core/config/lint.ts` |
...
| **Path B onboarding runs end to end, and so does the market.** A stranger with no ORCID registers an organisation on `/persona`; it lands in the same verification queue as the seeded requests and an administrator verifies or declines with a recorded reason, from the console (with a find box) or the organisation's own page. A pending Path B organisation may already act: onboarding runs in parallel, it does not gate the spine. From its organisation page, an organisation publishes and withdraws its own offers and needs (a signatory's act; contact details are stripped from the public fields), and its administrator gives colleagues seats at any of the four permissions and revokes them (a revoked seat stops acting and stays on the record, and the last administrator seat cannot be revoked). With `GENELINK_SEED=none` the instance starts empty, as a fresh database would, and the whole journey runs from nothing: register, publish, find, signal, verify, match, staff the seats, file. | `/persona`, `/organisations/<id>`, `/admin` |
...
| **Production-grade hygiene from the first commit.** Security headers (CSP, HSTS, frame denial), a non-root container, zero known dependency vulnerabilities, zero WCAG 2.2 AA violations under axe-core across every route (`npm run a11y` replays the sweep against any running instance), and a `/health` probe that verifies the audit chain and returns 503 if it is broken. The paths a visitor can use without a seat are limited per client (5 registrations, 30 journey declarations and 60 document checks every 10 minutes, keyed on Cloudflare's client-address header on the live host), so one script cannot lock newcomers out or grow memory without end. Refusals are recognised by a kind carried on the error, not by `instanceof`, so a refusal reads as its sentence and lands on the audit chain whichever server layer first loaded the core. Denied views answer with real statuses (401 anonymous, 403 wrong authority) so a refusal is visible to scanners and WAF rules, not only to a human reading the page. | `next.config.ts`, `Dockerfile`, `scripts/a11y-sweep.mjs`, `/health` |
...
| **Each side records its own acts, and the record decides when a step is done.** A party's signatory files, resubmits, appeals or withdraws; the administrator records the authority's acts and never files for a party. Where the law names the filer, the file says so and the platform holds to it: in Brazil the Brazilian registrant makes the cadastro and a foreign company never registers in its own name (Lei Art. 12 II; Decreto Art. 22-B), so the foreign party prepares and the Brazilian party's signatory records the filing. Filing is refused while scope is undetermined or escalated, or while a prohibition applies. A regulator stage is complete when the proceeding ends, a stage that ends in an instrument when the instrument is recorded, and new facts that block a completed stage reopen it. Clocks are evaluated whenever a case is read. | Any case's "Regulator processing" panel; `core/platform.ts` |
...
| Persistence | The free demo instance has an ephemeral filesystem. The MVP uses PostgreSQL with row-level security and an append-only audit table | `core/store/memory.ts` behind `core/store/Store.ts`. `core/store/postgres.ts` documents the adapter and is unimplemented. `GENELINK_SEED=none` runs the in-memory store empty, the state a fresh database starts in |
...
| Adviser and broker attachment | An MVP flow behind the same seat model | Marked on the case page and `/open-decisions` |
...
```bash
npm ci
npm run test:ci        # 172 tests on the core: schema, lint, scope, pathway, state machines, clocks and calendars, instruments, audit, full journeys, dry run for every country, a regime suite pinning each country file to its Appendix B diagram, a hardening suite replaying every defect found in live evaluation and in the final audit against the diagrams and primary texts, a real-world suite replaying defects found using the site as several people at once, some of them hostile, and a usage suite replaying what running the product as its users, on an empty instance and under load, surfaced
...
GENELINK_SEED=none npm run dev   # the same, starting empty: no personas, listings or cases
```
...
core/                         framework independent. No import from Next, React or a database driver.
  ...
  errors.ts                   the refusals a person is shown, recognised by kind across server layers
...
lib/rateLimit.ts              per-client limits on the paths a visitor can use without a seat
lib/describeError.ts          turns any failure into the one sentence the page shows; unexpected errors are logged, never leaked
